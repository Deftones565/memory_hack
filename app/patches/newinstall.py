...
def create_auth_file():
    ...
    if auth_file.exists():
        print("Auth file already exists, skipping user creation.")
        return

    print("\nSet up authentication for Memory Hack")
    username = input("Choose a username: ").strip()
    while not username:
        username = input("Username cannot be empty, choose again: ").strip()
    ...
    print("Saved credentials to auth.json")
...
def generate_ssl_keys():
    ...
    if cert_path.exists() and key_path.exists():
        print("SSL keys already exist, skipping generation.")
        return

    print("Generating self-signed SSL keys...")
